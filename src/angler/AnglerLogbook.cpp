...
void AnglerLogbook::Begin()
{
    prefs.begin("ang-log", false);
...
void AnglerLogbook::RecordCatch(long nowUtc, bool duringWindow)
{
...
uint32_t AnglerLogbook::CurrentStreak(long nowUtc) const
{
    const uint32_t day = (uint32_t)(nowUtc / DAY);
    return (day >= d.lastDay && day - d.lastDay <= 1) ? d.streak : 0;
}

uint32_t AnglerLogbook::TodayCount(long nowUtc) const
{
    const uint32_t day = (uint32_t)(nowUtc / DAY);
