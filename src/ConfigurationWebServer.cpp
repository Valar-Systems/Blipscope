        ...
        <fieldset class="border border-sky-400 p-5 w-full max-w-2xl mx-auto sm:m-10">
            ...
            <form id="cfg" action="/save" method="POST" class="flex flex-col gap-4 sm:gap-2">
                ...
                <fieldset class="border border-sky-400 p-3">
                    ...
                    <span class="text-xs text-sky-600 mt-1">ids: iss, launch, kp, solarwind, scales, flare, aurora, dsn, deepspace, humans, moon, eclipse, meteor, cosmic, splash, clock. Empty rotates all. Each screen appears only when its feed has data; clock/moon/eclipse/meteor/cosmic always show. aurora needs a location.</span>
                </fieldset>
...
void ConfigurationWebServer::Initialise() {
    ...
    server.on("/", HTTP_GET, [&](AsyncWebServerRequest* request) {
        ...
        const String spaceScreens = prefs.isKey("space-screens")
            ...
            : String("iss,launch,kp,solarwind,scales,flare,aurora,dsn,deepspace,humans,moon,eclipse,meteor,cosmic,clock");
#endif
        prefs.end();
