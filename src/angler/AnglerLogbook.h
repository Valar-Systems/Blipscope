...
// The Angler edition's persistent catch log: lifetime tallies of fish the user has logged (tap the
// Catch Log screen to record one), how many landed during a solunar feeding window, best single day,
// and a fishing-day streak. Its own NVS namespace ("ang-log"), mirroring SpaceLogbook / EamLogbook.
//
// The "bite-window hit rate" (share of catches recorded while a major/minor period was active) is the
// payoff stat: it tells the angler whether the solunar forecast is actually working for them.
//
